...
Mat horizontal_gradient(Mat img)
{
	
	Mat result(img.rows,img.cols,CV_8UC1,Scalar(0)); // image to store gradient data
	...
	Mat new_image(result.rows,result.cols,CV_8UC1,Scalar(0));
	...
	for(int i=0;i<result.rows;i++)
	{
		for(int j=0;j<result.cols;j++)
		{
			...
			else new_image.at<uchar>(i,j)=255;
		}
	}
	return new_image;
